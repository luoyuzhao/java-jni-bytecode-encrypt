...
static long32 oldf(ulong32 r, unsigned char subkey[8])//这个函数实现了R32的扩展排列~48位扩展结果与48位密钥异或~通过8个Sbox后还原成32位
{
	...
	*/
	...
	rt = (r >> 1) | ((r & 1) ? 0x80000000 : 0); /* [%] rt = ROR(rt, 1); *///循环右移1位
	//sbox_output_perm_table为8*64的表格/*[%] 8 * 64 * long32: 8 = 8 boxes 64 = 6 - bit input long32 = scattered 32 - bit output produced from 4 - bit original input
	rval = 0;
	/*[%]
	...
	//           6       2       8        4           0           6           2
	*/
	rval |= sbox_output_perm_table[0][((rt >> 26) ^ *subkey++) & 0x3F]; //取最右边6位, subkey[8][6]为密钥
	...
	rt = (r << 1) | ((r & 0x80000000) ? 1 : 0); /* [%] rt = ROL(r, 1); 1C 1D 1E 1F 20 1 *///循环左移1位
